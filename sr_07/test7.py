...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        self.canvas = tkinter.Canvas(window, width = self.vid.width, height = 500)
        self.canvas.grid(row=0, column=0, columnspan=4, rowspan=1, padx=5, pady=5, sticky='s')


        self.ybar = tkinter.Scrollbar(window, orient='vertical', command=self.canvas.yview)
        self.ybar.grid(row=0, column=0, columnspan=1, rowspan=1, sticky='nw')
        self.canvas.configure(yscrollcommand=self.ybar.set)

        # self.canvasFrame = tkinter.Frame(window, width = self.vid.width, height = 1500, bg="gray")
        # self.canvasFrame.grid(row=0, column=0, padx=5, pady=5, sticky='s')
        # self.canvasFrame.grid_propagate(False)
        #
        # self.canvas = tkinter.Canvas(self.canvasFrame, width = self.vid.width, height = 1500)
        # self.canvas.grid(row=0, column=1, columnspan=4, rowspan=1, padx=5, pady=5, sticky='s')
        #
        # self.ybar = tkinter.Scrollbar(self.canvasFrame, orient='vertical', command=self.canvasFrame.yview)
        # self.ybar.grid(row=0, column=0, columnspan=1, rowspan=1, sticky='ne')
        #
        #
        # self.canvasFrame.configure(yscrollcommand=self.ybar.set)

        # Create a combox for threshold
        self.combox = ttk.Combobox(window)
        # self.combox = ttk.Combobox(self.canvasFrame)
        ...
        # self.btn_snapshot=tkinter.Button(self.canvasFrame, text="图像获取", width=50, command=self.snapshot)
        ...
        # self.btn_imgprocess = tkinter.Button(self.canvasFrame, text="图像处理", width=50, command=self.img_process)
